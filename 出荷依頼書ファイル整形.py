...
def find_csv_path():
    ...
    for path in possible_paths:
        if os.path.exists(path):
            return path
    return None
def find_change_list_db_path():
    """
    Change_listテーブルを含むSQLite DBのパスを探索し、最初に見つかったものを返す。
    見つからない場合はNoneを返す。
    """
    username = get_username()
    possible_paths = [
...
def check_and_apply_update_on_start():
    """
    起動時に更新を確認する。
    更新が必要な場合は強制更新して True を返す（本体処理は進めない）。
    更新不要の場合は False を返す。
    """
    try:
        update_dir = find_update_directory()
        if not update_dir:
            return False
        ...
        latest_version_text = Path(version_file).read_text(encoding="utf-8").strip()
...
def load_change_list_mapping(db_path):
    """
    Change_listテーブルから
      key: Destination + Item_number
    ...
    with sqlite3.connect(db_path) as conn:
        cursor = conn.cursor()
        cursor.execute('SELECT Destination, Item_number, Source FROM Change_list')
        for destination, item_number, source in cursor.fetchall():
            destination_text = normalize_lookup_value(destination)
            item_number_text = normalize_lookup_value(item_number)
            key = f"{destination_text}{item_number_text}"
            if key:
                mapping[key] = (normalize_lookup_value(source), destination_text)
    return mapping
...
def process_file(input_file):
    ...
    try:
        ...
        if not db_path:
            messagebox.showerror("Error", "Change_list DB file was not found")
            return
        change_list_mapping = load_change_list_mapping(db_path)
        for row_index in range(1, ws.max_row + 1):
            g_text = normalize_lookup_value(ws.cell(row=row_index, column=7).value)
            b_text = normalize_lookup_value(ws.cell(row=row_index, column=2).value)
            if not g_text or not b_text:
                continue
            lookup_key = f"{g_text}{b_text}"
            matched = change_list_mapping.get(lookup_key)
            if matched:
                source_value, destination_value = matched
                g_cell = ws.cell(row=row_index, column=7)
                g_cell.value = f"{source_value}\n\uFF08MM\uff1a{destination_value}\uFF09"
                new_font = copy(g_cell.font)
                new_font.size = 10
                g_cell.font = new_font
                new_alignment = copy(g_cell.alignment)
                new_alignment.wrap_text = True
                g_cell.alignment = new_alignment
        lot_number_rows = []
        for row_obj in ws['F']:
...
def main():
    ...
    root = tk.Tk()
    root.withdraw()
    if check_and_apply_update_on_start():
        return
    if len(sys.argv) < 2:
        messagebox.showinfo("Info", "Please drop one or more Excel files onto this script.")
        return
